...
- 作業写真はGoogle Driveへ保存し、`_InstaCheckList_Photos` シートへ現在のシリアル番号、項目ID、Driveリンク、撮影日時、作業者名と共有用の小容量サムネイルを保存します。これにより、別の作業者が撮影した写真もアプリ内で確認できます。
- 保存先として、編集可能な「マイドライブの共有フォルダ」または「共有ドライブ内のフォルダ」を先に選択します。新規Spreadsheetと作業写真はそのフォルダ直下だけに作成します。
